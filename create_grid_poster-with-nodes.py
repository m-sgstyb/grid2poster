...
DEFAULT_THEMES: dict[str, dict[str, str]] = {
    "electric_midnight": {
        ...
        "line_extra": "#FFFFFF",
        "fade": "#06111F",
        "substation_fill": "#15324A",
        "substation_outline": "#8FC7F3",
        "substation_marker": "#D8EEFF",
        "plant_fill": "#3D2D16",
        "plant_outline": "#F6D28B",
        "plant_marker": "#FFE9B8",
    },
    "paper_grid": {
        "name": "Paper Grid",
        ...
        "line_extra": "#5B2016",
        "fade": "#F4EFE6",
        "substation_fill": "#D8E0D5",
        "substation_outline": "#436F75",
        "substation_marker": "#436F75",
        "plant_fill": "#EAD9B8",
        "plant_outline": "#A65C2E",
        "plant_marker": "#A65C2E",
    },
    "blackout": {
        "name": "Blackout",
        ...
        "line_extra": "#FFFFFF",
        "fade": "#050505",
        "substation_fill": "#111111",
        "substation_outline": "#CFCFCF",
        "substation_marker": "#FFFFFF",
        "plant_fill": "#222222",
        "plant_outline": "#FFFFFF",
        "plant_marker": "#FFFFFF",
    },
}


SITE_THEME_FALLBACK: dict[str, str] = {
    "substation_fill": "#15324A",
    "substation_outline": "#8FC7F3",
    "substation_marker": "#D8EEFF",
    "plant_fill": "#3D2D16",
...
class Theme:
    ...
    fade: str
    substation_fill: str
    substation_outline: str
    substation_marker: str
    plant_fill: str
    plant_outline: str
    plant_marker: str
    ...
    def from_dict(cls, raw: dict[str, str]) -> "Theme":
        merged = {**SITE_THEME_FALLBACK, **raw}
        required = {
            "name",
            "description",
            "bg",
        ...
        return cls(**{key: merged[key] for key in required})
...
def fetch_power_sites(
    country: str,
    boundary: gpd.GeoDataFrame,
    tile_size_km: float = 200,
    render_crs: str = "EPSG:8857",
) -> gpd.GeoDataFrame:
    """Fetch OSM power plants and substations using the same tiled query strategy as lines."""
    values = ["plant", "substation"]
    key = cache_key("power_sites", country, values, tile_size_km, render_crs)
    cached = cache_get(key)
    if cached is not None:
        print(f"Using cached power sites for {country}")
        return cached

    tiles = make_query_tiles(boundary, tile_size_km=tile_size_km, render_crs=render_crs)
    print(f"Downloading OSM power sites: power={values} across {len(tiles):,} tiles")

    frames: list[gpd.GeoDataFrame] = []
    tile_iterator = tqdm(
        enumerate(tiles.geometry, start=1),
        total=len(tiles),
        desc="Downloading power sites",
        unit="tile",
        leave=True,
    )

    for tile_number, tile_geom in tile_iterator:
        try:
            features = ox.features_from_polygon(tile_geom, tags={"power": values})
        except Exception as exc:
            print(f"  Warning: skipping site tile {tile_number:,} after Overpass error: {exc}")
            continue
        ...
        features = features.reset_index()
        usable = features[features.geometry.type.isin(["Point", "MultiPoint", "Polygon", "MultiPolygon"])].copy()
        if usable.empty:
            continue
        ...
        frames.append(gpd.GeoDataFrame(usable[keep_cols], geometry="geometry", crs="EPSG:4326"))

    if not frames:
        print(f"No power plants or substations found for {country}")
...
def prepare_power_sites(
    sites: gpd.GeoDataFrame,
    boundary: gpd.GeoDataFrame,
    output_crs: str,
) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame, gpd.GeoDataFrame, gpd.GeoDataFrame]:
    """Prepare plant/substation footprints and marker points for poster rendering."""
    ...
    sites_projected = sites.to_crs(output_crs)

    try:
        sites_projected = gpd.clip(sites_projected, boundary_projected)
    except Exception:
        pass

    sites_projected = sites_projected.explode(ignore_index=True)
    sites_projected = sites_projected[~sites_projected.geometry.is_empty].copy()
    if sites_projected.empty:
        return empty.copy(), empty.copy(), empty.copy(), empty.copy()

    area_mask = sites_projected.geometry.type.isin(["Polygon", "MultiPolygon"])
    areas = sites_projected[area_mask].copy()
    plant_areas = areas[areas["power"] == "plant"].copy()
    substation_areas = areas[areas["power"] == "substation"].copy()

    markers = sites_projected[sites_projected.geometry.type.isin(["Point", "MultiPoint", "Polygon", "MultiPolygon"])].copy()
    if markers.empty:
        return plant_areas, substation_areas, empty.copy(), empty.copy()

    markers["area_m2"] = markers.geometry.area.where(area_mask.reindex(markers.index, fill_value=False), 0.0)
    markers["geometry"] = markers.geometry.representative_point()
    markers["marker_size"] = 14 + np.clip(np.sqrt(markers["area_m2"].fillna(0)) / 160, 0, 95)

    if "voltage" in markers.columns:
        markers["voltage_kv"] = markers["voltage"].apply(parse_voltage_to_kv)
    else:
        markers["voltage_kv"] = np.nan

    plant_markers = markers[markers["power"] == "plant"].copy()
    substation_markers = markers[markers["power"] == "substation"].copy()

    if not plant_markers.empty:
        plant_markers["marker_size"] = plant_markers["marker_size"].clip(16, 120)
    if not substation_markers.empty:
        voltage_boost = substation_markers["voltage_kv"].fillna(0).clip(upper=500) / 28
        substation_markers["marker_size"] = (substation_markers["marker_size"] + voltage_boost).clip(10, 82)

    return plant_areas, substation_areas, plant_markers, substation_markers
...
def render_power_site_areas(
    ax: plt.Axes,
    plant_areas: gpd.GeoDataFrame,
    substation_areas: gpd.GeoDataFrame,
    theme: Theme,
) -> None:
    """Render soft site footprints underneath the transmission-line layer."""
    if not plant_areas.empty:
        plant_areas.plot(
            ax=ax,
            facecolor=theme.plant_fill,
            edgecolor=theme.plant_outline,
            linewidth=0.35,
            alpha=0.18,
            zorder=1.35,
        )

    if not substation_areas.empty:
        substation_areas.plot(
            ax=ax,
            facecolor=theme.substation_fill,
            edgecolor=theme.substation_outline,
            linewidth=0.28,
            alpha=0.15,
            zorder=1.40,
        )
...
def render_power_site_markers(
    ax: plt.Axes,
    plant_markers: gpd.GeoDataFrame,
    substation_markers: gpd.GeoDataFrame,
    theme: Theme,
) -> None:
    """Render glowing plant circles and substation diamonds above the transmission-line layer."""
    if not plant_markers.empty:
        x = plant_markers.geometry.x
        y = plant_markers.geometry.y
        size = plant_markers["marker_size"].to_numpy()
        ax.scatter(x, y, s=size * 3.8, c=theme.plant_marker, alpha=0.10, linewidths=0, zorder=3.6)
        ax.scatter(
            x,
            y,
            s=size,
            c=theme.plant_marker,
            alpha=0.92,
            marker="o",
            edgecolors=theme.bg,
            linewidths=0.25,
    ...
    if not substation_markers.empty:
        x = substation_markers.geometry.x
        y = substation_markers.geometry.y
        size = substation_markers["marker_size"].to_numpy()
        ax.scatter(x, y, s=size * 3.0, c=theme.substation_marker, alpha=0.10, linewidths=0, zorder=4.0)
        ax.scatter(
            x,
            y,
            s=size,
            c=theme.substation_marker,
            alpha=0.95,
            marker="D",
            edgecolors=theme.bg,
            linewidths=0.25,
...
def render_poster(
    ...
    render_power_site_areas(ax, plant_areas, substation_areas, theme)
    ...
    render_power_site_markers(ax, plant_markers, substation_markers, theme)
...
def main(argv: Iterable[str] = sys.argv[1:]) -> int:
    ...
    plant_areas, substation_areas, plant_markers, substation_markers = prepare_power_sites(
        raw_sites,
        boundary_wgs84,
        args.crs,
    )
    ...
    render_poster(
        ...
        substation_areas=substation_areas,
        plant_markers=plant_markers,
        substation_markers=substation_markers,
        theme=theme,
        width=args.width,
        height=args.height,
        output_file=out,
